...
from bp_assistant.ui import BpApplication
...
ERROR_LOG = APP_ROOT / "启动错误.log"


def enable_high_dpi() -> None:
    """让 Tk 在 Windows 高分屏上按真实像素渲染，避免系统二次拉伸模糊。"""
    dll_directory = None
    try:
        if hasattr(os, "add_dll_directory"):
            dll_directory = os.add_dll_directory(str(Path(sys.executable).resolve().parent))
        import ctypes
    ...
    try:
        if hasattr(ctypes, "windll"):
            try:
                ...
                set_app_id("LinkedOps.BPAssistant")
            except (AttributeError, OSError, TypeError):
                pass
            try:
                # Per-monitor v2 prevents Windows from bitmap-stretching the
                # whole Tk window when it is opened on a high-DPI display.
...
def main() -> None:
    try:
        ...
        app = BpApplication(operators, OPERATOR_DATA)
        try:
